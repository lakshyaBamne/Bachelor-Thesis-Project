...
def plot_density(grid: list, density: list[list], **kwargs) -> None:
    ...
    plt.plot(grid, density[-1], label="Final Density", color="red")

    plt.title(kwargs["MAIN_TITLE"])
    plt.legend()

    plt.savefig(f"PLOTS/SERIAL-{kwargs['MAIN_TITLE']}.png")
...
def main(MODE: str) -> None:
    ...
    FILE_GRID = f"{MODE}grid.txt"
    FILE_DENSITY = f"{MODE}density.txt"
    ...
    plot_density(grid, density, MAIN_TITLE=PROBLEM+"(SERIAL)")
